...
// TODOxDONE: Rect 받아오기.  Point는 터치이벤트로 처리할거라 필요없을지도..
//Point origin;
Rect selection;
...
RotatedRect tmpTrackBox;
RotatedRect getTrackBoxVal(){return tmpTrackBox;}
...
//    bool paused = false;
    ...
    if (!paused) {
        //TODOxDONE: RGB2HSV_FULL로 변경했는데 맞는지 확인 필요함(input 색상영역이 rgba라서 이렇게 했다.) (지금은 다시 BGR2HSV로)
        ...
        if (trackObject) {
            ...
            // Perform CAMShift
            calcBackProject(&hue, 1, 0, hist, backproj, &phranges); //TODOxDONE: 여기서 버그 Error: Assertion failed (dims > 0 && !hist.empty())
            backproj &= mask;
...
extern "C"
JNIEXPORT void JNICALL
Java_kr_ac_ssu_infocom_opencv_1contrib_1test_CAMShiftActivity2_camShift(JNIEnv *env,
                                                                        jobject instance,
                                                                        jlong matAddrInput,
                                                                        jlong matAddrResult,
                                                                        jintArray rectSelectionArray_) {
    ...
    // 셀렉트 오브젝트 boolean 값 자바단에서 가져온다
    jfieldID id_selectObject = env->GetFieldID(class_fieldcontrol, "selectObject", "Z");
    jboolean selectObject = env->GetBooleanField(instance, id_selectObject);

    //rect selection
    jint *ptrRectSelectionArray = env->GetIntArrayElements(rectSelectionArray_, NULL);
    Rect rectSelection(ptrRectSelectionArray[0], ptrRectSelectionArray[1],
                       ptrRectSelectionArray[2], ptrRectSelectionArray[3]);
    env->ReleaseIntArrayElements(rectSelectionArray_, ptrRectSelectionArray, 0);
...
Java_kr_ac_ssu_infocom_opencv_1contrib_1test_CAMShiftActivity2_nativeGetTrackBox(JNIEnv *env,
                                                                                 jclass type) {
    ...
    ptrArray[1] = trackBox.center.y;
    ptrArray[2] = trackBox.size.width;
    ptrArray[3] = trackBox.size.height;
    ptrArray[4] = trackBox.angle;
    env->ReleaseFloatArrayElements(valArray, ptrArray, 0);
